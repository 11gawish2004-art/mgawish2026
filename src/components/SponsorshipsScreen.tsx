...
export default function SponsorshipsScreen() {
  ...
  const handleAddDonor = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ name: '', phone: '', amount: '', notes: '' });
    } catch (err) {
  ...
  const toggleCollection = async (item: SponsorshipItem) => {
    try {
      if (item.paymentId) {
        ...
        await updateDoc(doc(db, 'sponsorship_payments', item.paymentId), {
        ...
        await addDoc(collection(db, 'sponsorship_payments'), {
          ...
          year: selectedYear,
          isCollected: true,
          amount: item.amount, // Default to donor's standard amount
          createdAt: serverTimestamp()
        });
      }
    } catch (err) {
  ...
  const stats = {
    ...
    totalCollected: filtered.filter(d => d.isCollected).reduce((acc, d) => acc + d.amount, 0),
    pending: filtered.filter(d => !d.isCollected).reduce((acc, d) => acc + d.amount, 0),
    ...
    <div className="p-4 lg:p-8 font-sans" dir="rtl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-emerald-500 p-6 rounded-3xl text-white shadow-lg shadow-emerald-50 flex items-center gap-4 border-b-4 border-emerald-600">
          ...
          <div>
            <p className="text-xs font-bold opacity-80">تم تحصيله هذا الشهر</p>
            <p className="text-2xl font-black tabular-nums">{stats.totalCollected.toLocaleString()} ج.م</p>
          </div>
        </div>

        <div className="bg-amber-500 p-6 rounded-3xl text-white shadow-lg shadow-amber-50 flex items-center gap-4 border-b-4 border-amber-600">
          <div className="w-12 h-12 bg-white/20 rounded-2xl flex items-center justify-center">
            <Clock className="w-6 h-6" />
          </div>
          <div>
            <p className="text-xs font-bold opacity-80">المتبقي للتحصيل</p>
            <p className="text-2xl font-black tabular-nums">{stats.pending.toLocaleString()} ج.م</p>
          </div>
        </div>
      </div>
          ...
          <div className="flex flex-col md:flex-row gap-4 flex-grow max-w-2xl">
            <div className="relative flex-grow">
              ...
              <input 
                ...
                className="w-full bg-white border border-emerald-50 p-4 pr-12 rounded-2xl font-bold outline-none focus:ring-2 ring-emerald-500/20 text-right text-emerald-950 shadow-inner"
              />
            </div>
            <select 
              className="bg-white border border-emerald-50 px-6 py-4 rounded-2xl font-bold outline-none focus:ring-2 ring-emerald-500/20 text-right text-emerald-950 shadow-inner"
              value={sortOrder}
          ...
          <table id="sponsorships-table-full" className="w-full text-right border-collapse min-w-[1000px] bg-white">
            <thead>
              <tr className="bg-emerald-50/50">
                <th className="px-6 py-5 text-emerald-900 font-black">اسم المتبرع</th>
                <th className="px-6 py-5 text-emerald-900 font-black text-center">المبلغ الشهري</th>
                <th className="px-6 py-5 text-emerald-900 font-black">رقم الهاتف</th>
                <th className="px-6 py-5 text-emerald-900 font-black text-center">الحالة هذا الشهر</th>
              ...
              {filtered.map(item => (
                <tr key={item.id} className={`hover:bg-emerald-50/20 transition-all group ${item.isCollected ? 'bg-emerald-50/10' : ''}`}>
                  <td className="px-6 py-5">
                      ...
                      {item.notes && <p className="text-xs text-stone-400 font-bold mt-1 line-clamp-1">{item.notes}</p>}
                  </td>
                  <td className="px-6 py-5 text-center">
                      <span className="inline-block bg-white border border-emerald-100 text-emerald-700 px-4 py-2 rounded-xl font-black tabular-nums shadow-sm">
                        {item.amount.toLocaleString()} ج.م
                      </span>
                  </td>
                  <td className="px-6 py-5">
                    <div className="flex flex-col gap-2">
                      <a href={`tel:${item.phone}`} className="flex items-center gap-3 font-bold text-stone-500 hover:text-emerald-600 transition-colors">
                        <div className="w-8 h-8 bg-stone-50 rounded-lg flex items-center justify-center text-emerald-600">
                          <Phone className="w-4 h-4" />
                        </div>
                        <span className="tabular-nums">{item.phone}</span>
                      </a>
                      <a 
                        href={`https://wa.me/+2${item.phone.startsWith('0') ? item.phone.substring(1) : item.phone}?text=${encodeURIComponent(`السلام عليكم، تذكير بخصوص كفالة شهر ${months[selectedMonth-1]} لـ ${item.name}`)}`}
                        target="_blank" 
                        rel="noreferrer"
                        className="flex items-center gap-2 font-bold text-emerald-600 hover:text-emerald-700 text-xs bg-emerald-50 px-2 py-1 rounded-lg w-fit"
                      >
                  ...
                  <td className="px-6 py-5 text-center">
                    <button 
                      ...
                      className={`px-6 py-3 rounded-2xl font-black text-sm transition-all flex items-center gap-2 mx-auto shadow-sm ${
                        item.isCollected 
                        ? 'bg-emerald-600 text-white hover:bg-emerald-700 shadow-emerald-50 shadow-inner' 
