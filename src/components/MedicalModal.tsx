                  ...
                  {activeTab === 'surgeries' && (
                    <>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <FormInput type="number" label="التكلفة" value={surgeryForm.cost.toString()} onChange={val => setSurgeryForm({...surgeryForm, cost: Number(val)})} icon={<Database className="w-4 h-4" />} />
                      </div>
        ...
        <div className="space-y-3">
          ...
          {type === 'radiology' && (
            <>
              ...
              {record.notes && <p className="text-xs text-emerald-800 italic bg-stone-50 p-2 rounded-xl">"{record.notes}"</p>}
            </>
          )}
        </div>
      </div>
